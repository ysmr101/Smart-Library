...
def test_create_author():
    ...
    assert response.status_code == 200
    assert response.json() == {"first_name": "John", "last_name": "Doe", "biography": "Mystery writer from the 1800s", "author_id": response.json()["author_id"]}

def test_create_already_registered_author():
    response = client.post(
        "/authors/",
        headers={"Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJBbm91ZCIsImV4cCI6MTcyMTMwMjE5MH0.N21BIcmrjsY-W6tQXHjIrxiDAtPz-PHrvFKRFiTesdg"},
        json={"first_name": "Jane", "last_name": "Austen", "biography": "English novelist known primarily for her six major novels which interpret, critique and comment upon the British landed gentry at the end of the 18th century."},
    )
...
def test_update_author():
    ...
    assert response.status_code == 200
    assert response.json() == {"author_id": response.json()["author_id"], "first_name": "Stephan", "last_name": "King", "biography": "Epic Horror legend"}
...
def test_add_user_preference():
    response = client.post("/users/me/preference/76c3eba2-fde9-44cc-81af-90ea9d8363b0",
